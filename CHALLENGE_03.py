claves_cifrados = """8-10 r: ozrcdfnug
9-10 q: hvsazxrigf
1-9 j: bbqonxjt
...
lineas = claves_cifrados.split('\n')

# Crear un diccionario para almacenar los datos organizados
diccionario_datos = []

# Procesar cada línea y agregarla al diccionario
for linea in lineas:
    if not linea:
        continue  # Ignorar líneas vacías
    partes = linea.split(' ')
    diccionario_datos.append({'rango': partes[0], 'letra':partes[1][0], 'cadena': partes[2]})

lista_aceptadas = []
lista_denegadas = []
# Acceder al valor de la clave 'rango' en el primer diccionario de la lista
for claves_cifrado in diccionario_datos:
    valor_rango = claves_cifrado['rango']
    valor_letra = claves_cifrado['letra']
    valor_cadena = claves_cifrado['cadena']
    inicio, fin = map(int,valor_rango.split('-'))
    if inicio <= valor_cadena.count(valor_letra) <= fin:
        lista_aceptadas.append(claves_cifrado)
    else:
        lista_denegadas.append(claves_cifrado)


# Imprimir lista de aceptadas con colores
print("Lista de aceptadas:")
for claves_cifrado in lista_aceptadas:
    print(Fore.GREEN +f"{claves_cifrado['rango']} {claves_cifrado['letra']}: {claves_cifrado['cadena']}" + Style.RESET_ALL)

# Imprimir lista de denegadas con colores
print("Lista de denegadas:")
for claves_cifrado in lista_denegadas:
    print(Fore.RED +f"{claves_cifrado['rango']} {claves_cifrado['letra']}: {claves_cifrado['cadena']}" + Style.RESET_ALL)
